...
const transformObje = (obj)=>{
    let newObj = [];
    obj.forEach(element => {
      newObj.push({
        code: objKey(element)[0],
        name: Object.entries(element)[0][1]
      });
    });
    console.log(newObj);
    return newObj;
...
exports.digitalCurrency = (req, res, next) => {
    ...
    request(options, (error, response, body) => {
        ...
        try{
           const bodyData = transformObje(JSON.parse(body).digital_currencies);
           res.status(201).json({ message: 'tickers', data: bodyData});
        } catch(err){
            const error = new Error('Error');
            error.statusCode = 500;
            error.data = err;
